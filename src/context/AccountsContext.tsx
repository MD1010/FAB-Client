...
const updateAccountStatus = (
  ...
  if (accountToUpdate) accountToUpdate.status = status;
  return newAcc;
};
...
const filterUpdated = (
  ...
  const accIndex = eaAccounts.findIndex((ea) => ea.email === accountId);
  const newEaAcc = [...eaAccounts];
  let existingfilter = newEaAcc[accIndex].filters.find(
    (f) => f.id === filter.id
  );
...
const filterDeleted = (
  ...
  const accIndex = eaAccounts.findIndex((ea) => ea.email === accountId);
  const filterIndex = eaAccounts[accIndex]?.filters.findIndex(
    (f) => f.id === filterId
  );
  const newEaAcc = [...eaAccounts];
