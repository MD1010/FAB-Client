...
const CardFilter: FunctionComponent<CardFilterProps>  = ({ accountId, filter, isChosen, onFilterChosen }) => {
    ...
    const deleteFilter = () => {
        accountsContext.dispatch({type: ACCOUNTS_ACTIONS.DELETE_ACCOUNT_FILTER, payload: {accountId , filterId: filter.id}});
    }
    ...
    const cardFilterStyle: CSSProperties = {
        ...
        boxShadow: `0px ${isRunnable? '-10px' : '0px'} 1px ${isRunnable? "green" : "gray"}`,
        borderRadius: "5px",
        margin: "10px"
    };
    ...
    return (
        <>
            {isfilterEditOpen? 
                <Modal open={isfilterEditOpen} onClose={() => setIsfilterEditOpen(false)}>
                    <>
                        <FilterModal filter={filter} accountId={accountId} onFilterSet={() => setIsfilterEditOpen(false)}/>
                    </>
                </Modal>
            : null 
            }
            <div style={cardFilterStyle} className="filter-container" onMouseEnter={e => setIsFilterOptionsShown(true)} onMouseLeave={e => setIsFilterOptionsShown(false)}>
                <div style={filterMoreStyle} className="filter-more" onClick={cardChosen}>
                    <EditIcon className="click-icon edit" onClick={() => setIsfilterEditOpen(true)}/>
                    <DeleteIcon className="click-icon remove" onClick={deleteFilter}/>
                </div>
                <div className="filter-details">
                    {filter instanceof PlayerFilter? PlayerFilterData() : ConsumableFilterData()}
                </div>
            </div>
        </>
    );
