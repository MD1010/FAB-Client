...
const PlayerTab: FunctionComponent<PlayerTabProps>  = ({ accountId, filter, onFilterSet }) => {
    ...
    const comboBoxes = new Map<string, FilterComboBox>([
        ...
        ["Nation", new FilterComboBox("Nation", entitiesContext.nations, filter?.nation)],
        ["League", new FilterComboBox("League", entitiesContext.leagues,  filter?.league)],
        ["Club", new FilterComboBox("Club", entitiesContext.teams, filter?.club)],
        ["ChemistryType", new FilterComboBox("ChemistryType", [], filter?.chemType)]
    ]);
    ...
    const setFilterHandler = () => {
        ...
        const newFilter = new PlayerFilter(filterId, 
                                            "",
        ...
        const actionType = filter === null? ACCOUNTS_ACTIONS.ADD_ACCOUNT_FILTER : ACCOUNTS_ACTIONS.UPDATE_ACCOUNT_FILTER;
        accountsContext.dispatch({type: actionType, payload: {accountId, filter: newFilter }});
        onFilterSet();
    }
    ...
    return (
        <>
            ...
            <div className="filter-row">
                <ModalCombobox comboBox={comboBoxes.get("Position")} onValueChanged={updateSelectedValue} />
                <ModalCombobox comboBox={comboBoxes.get("Quality")} onValueChanged={updateSelectedValue} />
            </div>
            <div className="filter-row">
                <ModalCombobox comboBox={comboBoxes.get("Nation")} onValueChanged={updateSelectedValue} />
                <ModalCombobox comboBox={comboBoxes.get("League")} onValueChanged={updateSelectedValue} />
            </div>
            <div className="filter-row">
                <ModalCombobox comboBox={comboBoxes.get("Club")} onValueChanged={updateSelectedValue} />
                <ModalCombobox comboBox={comboBoxes.get("ChemistryType")} onValueChanged={updateSelectedValue} />
            </div>
